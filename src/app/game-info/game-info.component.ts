...
export class GameInfoComponent implements OnChanges {
  ...
  ngOnChanges() {
    if (this.card) {
      this.cardSrc = this.card?.split('_')[1];
      this.cardNumber = parseInt(this.cardSrc);
      this.title = this.cardAction[this.cardNumber - 1].title;
      this.description = this.cardAction[this.cardNumber - 1].description;

      // hearts_6 wird beim _ gesplittet und wird durch splitNsName() zu einem Array. In dem array ['hearts', '6']. Wenn man auf die Stele 1 zugreift, hat man nur die '6'.}
    }
